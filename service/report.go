...
func RequestReport() (obj model.RequestReportResponse, err error) {
	...
	err = CommonHandle("POST", "RequestReport", "/", res.MarketplaceID, res, mp, true, &obj)
	if err != nil {
...
func GetReportList() (obj model.GetReportListResponse, err error) {
	...
	mp["MaxCount"] = "1"                                                     //可以不用
	err = CommonHandle("POST", "GetReportList", "/", res.MarketplaceID, res, mp, true, &obj)
	if err != nil {
		fmt.Println(err.Error())
	} else {
...
func GetReport() (list []model.GetMerchantListingsAllData, err error) {
	...
	err = CommonHandle("POST", "GetReport", "/", res.MarketplaceID, res, mp, false, &list)
	if err != nil {
		fmt.Println(err.Error())
	} else {
